...
fun TileMapView(
    ...
    fun recenter() {
        val (tx, ty) = lonLatToTileF(initialLon, initialLat, zoom)
        centerTileX = tx
        centerTileY = ty
    }
    ...
    Box(modifier = modifier.fillMaxSize()) {
        Canvas(
            ...
            if (!routePoints.isNullOrEmpty()) {
                ...
                drawPath(path, color = RouteColor, style = Stroke(width = 9f, cap = androidx.compose.ui.graphics.StrokeCap.Round))
            }
        ...
        Column(
            ...
            MapControlButton(icon = Icons.Filled.MyLocation, contentDescription = "Recenter", onClick = { recenter() })
        }
    }
}
