...
final class AuthSessionViewModel: NSObject, ObservableObject, ASWebAuthenticationPresentationContextProviding {
    ...
    func start(
        ...
        print("[Quail] AuthSessionViewModel.start() startURL=\(startURL.absoluteString)")
        ...
        let authSession = ASWebAuthenticationSession(
            ...
            if let callbackURL {
                print("[Quail] AuthSessionViewModel.callbackURL=\(callbackURL.absoluteString)")
                ...
                if !token.isEmpty {
                    ...
                    print("[Quail] AuthSessionViewModel authenticated email=\(email) tenantID=\(tenantID ?? 0)")
                    onAuthenticated()
                    return
                }
                print("[Quail] AuthSessionViewModel callback missing token")
            }

            if let nsError = error as NSError?, nsError.domain == ASWebAuthenticationSessionError.errorDomain {
                print("[Quail] AuthSessionViewModel ASWebAuthenticationSession error code=\(nsError.code) desc=\(nsError.localizedDescription)")
                ...
                print("[Quail] AuthSessionViewModel non-session error=\(error?.localizedDescription ?? "nil")")
        ...
        print("[Quail] AuthSessionViewModel.start() returned \(started)")
    }
