...
final class FinancingStore: ObservableObject {
    ...
    func refresh() async {
        ...
        plans = (try? await QuailAPI.shared.fetchFinancingPlans()) ?? []
    }

    func createPlan(label: String, totalAmount: Double, totalMonths: Int, transactionId: String?) async {
        guard let plan = try? await QuailAPI.shared.createFinancingPlan(
    ...
    func recordPayment(_ plan: FinancingPlan) async {
        try? await QuailAPI.shared.recordFinancingPayment(planId: plan.id)
        await refresh()
    }

    func deletePlan(_ plan: FinancingPlan) async {
        try? await QuailAPI.shared.deleteFinancingPlan(planId: plan.id)
