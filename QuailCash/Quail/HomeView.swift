    ...
    var body: some View {
        ...
        .sheet(isPresented: $model.showAuthSheet) {
            AuthSessionView(
                ...
                onAuthenticated: {
                    print("[Quail] Auth sheet reported authenticated")
                    model.finishAuthentication()
                },
                onCancel: {
                    print("[Quail] Auth sheet cancelled")
        ...
        .sheet(item: $verifyAccountSheet) { account in
            VerifyBalanceSheetView(
                ...
                onConfirm: { dateISO in
                    _ = try await QuailAPI.shared.verifyAccountBalance(accountID: account.id, verifiedDate: dateISO)
                    await model.reload()
        ...
        .onChange(of: model.showAuthSheet) { _, isShowing in
            if isShowing {
                print("[Quail] Presenting auth sheet")
            }
        }
    }
...
final class HomeViewModel: ObservableObject {
    ...
    func startIfNeeded() {
        ...
        print("[Quail] HomeViewModel.startIfNeeded()")
        Task { await reload() }
    }
    ...
    func reload() async {
        ...
        print("[Quail] HomeViewModel.reload() begin")
        do {
            extraSaved = nil
            ...
            if let credit = home?.bankTotals.credit {
                ...
                print("[Quail] HomeViewModel.creditTotals total=\(credit.total) accounts=[\(accounts)]")
            }
            statusText = "Loaded real data."
            print("[Quail] HomeViewModel.reload() success")
        } catch QuailAPIError.unauthorized {
            ...
            upcomingLoading = false
            upcomingError = nil
            print("[Quail] HomeViewModel.reload() unauthorized -> needsAuthentication")
        } catch {
            errorMessage = error.localizedDescription
            ...
            upcomingLoading = false
            upcomingError = nil
            print("[Quail] HomeViewModel.reload() error: \(error.localizedDescription)")
        }
        isLoading = false
        print("[Quail] HomeViewModel.reload() end")
    }

    func finishAuthentication() {
        print("[Quail] HomeViewModel.finishAuthentication()")
        showAuthSheet = false
    ...
    func cancelAuthentication() {
        print("[Quail] HomeViewModel.cancelAuthentication()")
        showAuthSheet = false
        needsAuthentication = true
    }

    func recalculateDailySnapshot() async {
        print("[Quail] HomeViewModel.recalculateDailySnapshot() begin")
        ...
        print("[Quail] HomeViewModel.recalculateDailySnapshot() end")
    }
}
...
private struct IncomeBreakdownPopupView: View {
    ...
    private func load() async {
        isLoading = true
        do {
            payload = try await QuailAPI.shared.fetchMonthBudget()
            loadError = nil
        } catch {
...
private struct SpentBreakdownPopupView: View {
    ...
    private func load() async {
        isLoading = true
        do {
            payload = try await QuailAPI.shared.fetchSpentSoFarBreakdown(start: nativeIsoMonthStart(), end: nativeIsoToday())
            loadError = nil
        } catch {
            loadError = error.localizedDescription
        }
    ...
    private func toggleCategory(_ category: String) async {
        ...
        do {
            transactionsByCategory[category] = try await QuailAPI.shared.fetchSpentSoFarTransactions(
                category: category,
...
private struct ExtraSavedBreakdownPopupView: View {
    ...
    private func load() async {
        isLoading = true
        do {
            payload = try await QuailAPI.shared.fetchExtraSavedDetail()
            loadError = nil
        } catch {
            loadError = error.localizedDescription
        }
...
private struct BankInfoPopupView: View {
    ...
    private func load() async {
        do {
            let next = try await QuailAPI.shared.fetchBankInfo()
            await MainActor.run {
                payload = next
                loadError = nil
    ...
    private func saveRate() async {
        ...
        do {
            _ = try await QuailAPI.shared.setInterestRate(accountID: selectedAccountID, ratePercent: rate, effectiveDate: effectiveDate, note: note)
            saveMessage = "Saved."
            onRefresh()
            await load()
        } catch {
...
private struct TransactionInspectPopupView: View {
    ...
    private func load() async {
        do {
            let next = try await QuailAPI.shared.fetchTransactionDetail(txId: transaction.id)
            await MainActor.run {
                detail = next
    ...
    private func saveCategory() async {
        ...
        defer { isSaving = false }
        do {
            _ = try await QuailAPI.shared.updateTransactionCategory(txId: transaction.id, category: categoryText)
            saveStatus = "Saved."
            onRefresh()
            await load()
        } catch {
    ...
    private func saveMeta() async {
        ...
        defer { isSaving = false }
        do {
            _ = try await QuailAPI.shared.updateTransactionMeta(txId: transaction.id, status: statusText, postedDate: postedDateText)
            saveStatus = "Saved."
            metaEditing = false
            onRefresh()
            await load()
    ...
    private func toggleIgnore() async {
        do {
            ...
            _ = try await QuailAPI.shared.ignoreTransaction(txId: transaction.id, ignored: next)
    ...
    private func invertAmount() async {
        do {
            _ = try await QuailAPI.shared.invertTransactionAmount(txId: transaction.id)
    ...
    private func deleteTransaction() async {
        do {
            _ = try await QuailAPI.shared.deleteTransaction(txId: transaction.id)
...
private struct CsvImportSheetView: View {
    ...
    private func loadAccounts() async {
        ...
        do {
            let payload = try await QuailAPI.shared.fetchBankInfo()
            bankInfo = payload
            if selectedAccountID == 0 {
    ...
    private func saveMapping() async {
        ...
        do {
            ...
            try await QuailAPI.shared.saveCsvMappingPreset(accountID: selectedAccountID, preset: payload)
    ...
    private func loadPreview() async {
        ...
        defer { workKind = nil }
        do {
            let next = try await QuailAPI.shared.fetchCsvPreview(fileURL: selectedFileURL)
            preview = next
            dryRun = nil
            importResult = nil
            tab = .setup
    ...
    private func runDryRun() async {
        ...
        defer { workKind = nil }
        do {
            dryRun = try await QuailAPI.shared.runCsvDryRun(fileURL: selectedFileURL, fields: mappingFields())
            tab = .dryRun
    ...
    private func runImport() async {
        ...
        defer { workKind = nil }
        do {
            importResult = try await QuailAPI.shared.importCsvMapped(fileURL: selectedFileURL, fields: mappingFields())
            let preset = currentPresetPayload
            activePreset = preset
            activePresetAccountID = selectedAccountID
            presetCacheByAccount[selectedAccountID] = preset
    ...
    private func loadPreset(for accountID: Int) async -> Bool {
        ...
        do {
            let preset = try await QuailAPI.shared.fetchCsvMappingPreset(accountID: accountID)
    ...
    private func isTimeoutError(_ error: Error) -> Bool {
        if let apiError = error as? QuailAPIError {
            switch apiError {
...
private struct UnassignedWizardSheetView: View {
    ...
    private func loadCategories() async {
        do {
            categories = try await QuailAPI.shared.fetchCategories()
        } catch {
            message = error.localizedDescription
        }
    }
    ...
    private func loadRows(resetIndex: Bool) async {
        ...
        do {
            rows = try await QuailAPI.shared.fetchUnassigned(limit: 25, mode: mode)
    ...
    private func saveRule() async {
        ...
        defer { isLoading = false }
        do {
            let job = try await QuailAPI.shared.createCategoryRule(category: category, keywords: keywords, applyNow: true)
            if let job {
    ...
    private func flushDeferredRules() async throws {
        ...
        for rule in queued {
            do {
                if let job = try await QuailAPI.shared.createCategoryRule(category: rule.category, keywords: rule.keywords, applyNow: true) {
                    try await waitForApplyJob(job.id)
                }
            } catch {
        ...
        if !failures.isEmpty {
            ...
            throw NSError(domain: "QuailAPI", code: 3, userInfo: [NSLocalizedDescriptionKey: "Failed to save \(failures.count) deferred rule(s)."])
        }
    }
    ...
    private func waitForApplyJob(_ jobID: Int) async throws {
        ...
        while Date() < deadline {
            let job = try await QuailAPI.shared.fetchCategoryRuleJob(jobID: jobID)
            ...
            if status == "failed" {
                throw NSError(domain: "QuailAPI", code: 1, userInfo: [NSLocalizedDescriptionKey: job.error ?? "Rule apply failed."])
            }
            try await Task.sleep(nanoseconds: 1_200_000_000)
        }
        throw NSError(domain: "QuailAPI", code: 2, userInfo: [NSLocalizedDescriptionKey: "Rule apply timed out."])
    }
