...
final class MobilePushManager: NSObject, ObservableObject {
    ...
    func registerDeviceToken(_ data: Data) {
        ...
        deviceToken = token
        Task {
            try? await QuailAPI.shared.registerIOSPushDevice(
                token: token,
    ...
    func unregisterCurrentDevice() async {
        ...
        try? await QuailAPI.shared.unregisterIOSPushDevice(token: deviceToken)
    }

    func handleRegistrationFailure(_ error: Error) {
        print("[Quail] APNs registration failed: \(error.localizedDescription)")
    }
...
final class QuailAppDelegate: NSObject, UIApplicationDelegate {
    func application(
