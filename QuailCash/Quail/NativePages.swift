...
private final class DashboardGlanceModel: ObservableObject {
    ...
    func load() async {
        do {
            let home = try await QuailAPI.shared.fetchHome(txLimit: 1)
...
private final class DashboardAuthModel: ObservableObject {
    ...
    func checkAuth() async {
        do {
            _ = try await QuailAPI.shared.fetchNotificationsUnreadCount()
            isSignedIn = true
...
private struct FinanceNotificationsContent: View {
    ...
    private func sendTestPush() async {
        ...
        do { try await QuailAPI.shared.sendIOSTestPush(); statusMessage = "Test push sent." }
...
private struct BudgetPageView: View {
    ...
    private func load() async {
        isLoading = true
        do {
            async let budget = QuailAPI.shared.fetchMonthBudget()
            async let saved = QuailAPI.shared.fetchExtraSaved()
...
private struct AllTransactionsPageView: View {
    ...
    private func load() async {
        isLoading = true
        do {
            transactions = try await QuailAPI.shared.fetchTransactions(limit: 100)
            errorMessage = nil
        } catch {
            errorMessage = error.localizedDescription
        }
...
private struct BankInfoPageView: View {
    ...
    private func load() async {
        isLoading = true
        do {
            payload = try await QuailAPI.shared.fetchBankInfo()
            errorMessage = nil
        } catch {
            errorMessage = error.localizedDescription
        }
...
private final class RegexRulesViewModel: ObservableObject {
    ...
    func reload(reset: Bool) async {
        ...
        errorMessage = nil
        do {
            let payload = try await QuailAPI.shared.fetchCategoryRuleList(
                ruleID: filterRuleID,
    ...
    func runFullCheck() async {
        ...
        isRunningCheck = true
        do {
            let payload = try await QuailAPI.shared.runCategoryRulesCheckAll(uncategorizedOnly: uncategorizedOnly)
    ...
    func saveRule(ruleID: Int, category: String) async {
        do {
            let response = try await QuailAPI.shared.updateCategoryRule(ruleID: ruleID, category: category)
    ...
    func setRuleActive(ruleID: Int, isActive: Bool) async {
        do {
            try await QuailAPI.shared.setCategoryRuleActive(ruleID: ruleID, isActive: isActive)
    ...
    func deleteRule(ruleID: Int) async {
        do {
            try await QuailAPI.shared.deleteCategoryRule(ruleID: ruleID)
...
private struct RegexTestSheetView: View {
    ...
    private func runTest() async {
        ...
        do {
            let payload = try await QuailAPI.shared.testCategoryRule(
                pattern: pattern,
                flags: flags,
