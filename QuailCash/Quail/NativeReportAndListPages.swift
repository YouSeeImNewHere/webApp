...
private final class NativeAnalyticsViewModel: ObservableObject {
    ...
    func load() async {
        ...
        do {
            let data = try await QuailAPI.shared.fetchData(
    ...
    func exportPDF() {
        ...
        controller.printInfo = {
            ...
            info.jobName = "Quail \(report.month ?? monthKey)"
            return info
        }()
...
private final class NativeAllTransactionsViewModel: ObservableObject {
    ...
    func loadPage(force: Bool, replace: Bool) async {
        ...
        do {
            let data = try await QuailAPI.shared.fetchData(path: "/transactions-all", queryItems: query)
    ...
    func loadFilterOptions() async {
        ...
        do {
            async let bankData = QuailAPI.shared.fetchData(path: "/bank-info")
            async let categoriesData = QuailAPI.shared.fetchData(path: "/categories")
    ...
    func saveTransaction() async {
        ...
        addTxMessage = "Saving..."
        do {
            try await QuailAPI.shared.sendJSON(path: "/transaction", method: "POST", jsonBody: [
...
private struct NativeTransactionInspectView: View {
    ...
    private func saveMeta() async {
        do {
            _ = try await QuailAPI.shared.updateTransactionMeta(txId: transaction.id, status: statusText, postedDate: dateText)
            actionStatus = "Saved."
            onRefresh()
        } catch {
            actionStatus = error.localizedDescription
...
private final class NativeRecurringViewModel: ObservableObject {
    ...
    func loadAll() async {
        ...
        async let recurringData = QuailAPI.shared.fetchData(path: "/recurring", queryItems: [
            URLQueryItem(name: "min_occ", value: minOccurrences),
            URLQueryItem(name: "include_stale", value: includeStale ? "true" : "false"),
        ])
        async let calendarData = QuailAPI.shared.fetchData(path: "/recurring/calendar", queryItems: [
    ...
    func loadIgnored() async {
        do {
            let data = try await QuailAPI.shared.fetchData(path: "/recurring/ignored-preview", queryItems: [
                URLQueryItem(name: "min_occ", value: minOccurrences),
                URLQueryItem(name: "include_stale", value: includeStale ? "true" : "false"),
            ])
    ...
    func ignoreMerchant(_ merchant: String) async {
        do {
            try await QuailAPI.shared.sendJSON(path: "/recurring/ignore/merchant", method: "POST", queryItems: [URLQueryItem(name: "name", value: merchant)])
            await loadAll()
    ...
    func unignoreMerchant(_ merchant: String) async {
        do {
            try await QuailAPI.shared.sendJSON(path: "/recurring/unignore/merchant", method: "POST", queryItems: [URLQueryItem(name: "name", value: merchant)])
            await loadIgnored()
            await loadAll()
        } catch { statusText = error.localizedDescription }
    }
    ...
    func ignorePattern(_ pattern: NativeRecurringPattern) async {
        do {
            try await QuailAPI.shared.sendJSON(path: "/recurring/ignore/pattern", method: "POST", queryItems: [
    ...
    func mergeSelected() async {
        ...
        do {
            try await QuailAPI.shared.sendJSON(path: "/recurring/merge-patterns-selected", method: "POST", jsonBody: [
                "merchant": mergeAlias,
                "pattern_keys": keys,
            ])
    ...
    private func mergePaycheckEvents(into calendar: inout NativeRecurringCalendarPayload, year: Int, month: Int) async {
        do {
            let profilePayload = try await QuailAPI.shared.fetchLESProfile()
            ...
            let paychecks = try await QuailAPI.shared.fetchLESPaychecks(year: year, month: month, profile: profile)
