    ...
    var body: some View {
        VehicleSheetContainer(title: "Vehicle Profile", palette: palette, onDismiss: { dismiss() }) {
            ...
            Button {
                ...
                store.save()
                let snap = QuailAPI.VehicleProfilePayload(
                ...
                Task { try? await QuailAPI.shared.saveVehicleProfile(snap) }
                dismiss()
            } label: {
                Text("Save")
...
struct VehiclePairTransactionsSheet: View {
    ...
    @State private var fuel: [QuailAPI.VehicleFuelPayload] = []
    @State private var maintenance: [QuailAPI.VehicleMaintenancePayload] = []
    @State private var selectedKind: String = "fuel"
    @State private var expandedRecordId: Int? = nil
    @State private var candidates: [QuailAPI.VehicleTxCandidate] = []
    ...
    private func loadRecords() async {
        do {
            fuel = try await QuailAPI.shared.fetchVehicleFuel(limit: 500)
            maintenance = try await QuailAPI.shared.fetchVehicleMaintenance(limit: 500)
        } catch {
    ...
    private func loadCandidates(kind: String, id: Int) async {
        ...
        do {
            candidates = try await QuailAPI.shared.fetchVehicleTxCandidates(kind: kind, recordId: id)
        } catch {
            statusMessage = "Failed to load candidates."
        }
    }

    private func linkTx(rec: AnyVehicleRecord, tx: QuailAPI.VehicleTxCandidate) async {
        do {
            try await QuailAPI.shared.linkVehicleTransaction(kind: rec.kind, recordId: rec.id, transactionId: tx.id, merchant: tx.merchant)
    ...
    private func unlinkTx(rec: AnyVehicleRecord) async {
        do {
            try await QuailAPI.shared.unlinkVehicleTransaction(kind: rec.kind, recordId: rec.id)
    ...
    private func runImport() async {
        ...
        do {
            ...
            if !parsedFuel.isEmpty {
                let n = try await QuailAPI.shared.bulkImportVehicleFuel(parsedFuel)
                messages.append("\(n) fuel records imported.")
            }
            if !parsedMaintenance.isEmpty {
                let n = try await QuailAPI.shared.bulkImportVehicleMaintenance(parsedMaintenance)
