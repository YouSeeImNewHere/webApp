...
enum QuailAPIError: LocalizedError {
    case unauthorized
    case badResponse
    case decodingFailed
...
final class QuailAPI {
    static let shared = QuailAPI()
    ...
    func fetchHome(txLimit: Int = 15) async throws -> HomePayload {
        ...
        print("[Quail] QuailAPI.fetchHome url=\(request.url?.absoluteString ?? "nil")")
        let (data, response) = try await perform(request)
        guard let http = response as? HTTPURLResponse else {
            throw QuailAPIError.badResponse
        }
        print("[Quail] QuailAPI.fetchHome status=\(http.statusCode) bytes=\(data.count)")
        guard http.statusCode == 200 else {
            if http.statusCode == 401 || http.statusCode == 403 {
                throw QuailAPIError.unauthorized
            }
            throw QuailAPIError.badResponse
        }
        ...
        do {
            ...
            if let text = String(data: data, encoding: .utf8) {
                print("[Quail] QuailAPI.decodeFailure bodyPrefix=\(String(text.prefix(800)))")
            } else {
                print("[Quail] QuailAPI.decodeFailure body is not utf8")
            }
            if let decodingError = error as? DecodingError {
                print("[Quail] QuailAPI.decodingError=\(Self.describe(decodingError))")
            } else {
                print("[Quail] QuailAPI.decodeFailure error=\(error.localizedDescription)")
            }
            throw QuailAPIError.decodingFailed
        }
    }
    ...
    func fetchTransactions(limit: Int = 100) async throws -> [TransactionItem] {
        ...
        print("[Quail] QuailAPI.fetchTransactions url=\(request.url?.absoluteString ?? "nil")")
        let (data, response) = try await perform(request)
        guard let http = response as? HTTPURLResponse else {
            throw QuailAPIError.badResponse
        }
        print("[Quail] QuailAPI.fetchTransactions status=\(http.statusCode) bytes=\(data.count)")
        guard http.statusCode == 200 else {
            if http.statusCode == 401 || http.statusCode == 403 {
                throw QuailAPIError.unauthorized
            }
        ...
        do {
            ...
            if let text = String(data: data, encoding: .utf8) {
                print("[Quail] QuailAPI.transactionsDecodeFailure bodyPrefix=\(String(text.prefix(800)))")
            }
            throw QuailAPIError.decodingFailed
        }
    }
    ...
    func fetchTransactionsAll(limit: Int = 10000, offset: Int = 0) async throws -> [TransactionItem] {
        ...
        print("[Quail] QuailAPI.fetchTransactionsAll url=\(request.url?.absoluteString ?? "nil")")
        let (data, response) = try await perform(request)
        guard let http = response as? HTTPURLResponse else {
            throw QuailAPIError.badResponse
        }
        print("[Quail] QuailAPI.fetchTransactionsAll status=\(http.statusCode) bytes=\(data.count)")
        guard http.statusCode == 200 else {
            if http.statusCode == 401 || http.statusCode == 403 {
                throw QuailAPIError.unauthorized
            }
        ...
        do {
            ...
            if let text = String(data: data, encoding: .utf8) {
                print("[Quail] QuailAPI.transactionsAllDecodeFailure bodyPrefix=\(String(text.prefix(800)))")
            }
            throw QuailAPIError.decodingFailed
        }
    }
    ...
    func fetchExtraSaved() async throws -> Double {
        ...
        print("[Quail] QuailAPI.fetchExtraSaved url=\(request.url?.absoluteString ?? "nil")")
        let (data, response) = try await perform(request)
        guard let http = response as? HTTPURLResponse else {
            throw QuailAPIError.badResponse
        }
        print("[Quail] QuailAPI.fetchExtraSaved status=\(http.statusCode) bytes=\(data.count)")
        guard http.statusCode == 200 else {
            if http.statusCode == 401 || http.statusCode == 403 {
                throw QuailAPIError.unauthorized
            }
        ...
        do {
            ...
            if let text = String(data: data, encoding: .utf8) {
                print("[Quail] QuailAPI.extraSavedDecodeFailure bodyPrefix=\(String(text.prefix(800)))")
            }
            throw QuailAPIError.decodingFailed
        }
    }
    ...
    func fetchExtraSavedDetail() async throws -> ExtraSavedDetailPayload {
        ...
        print("[Quail] QuailAPI.fetchExtraSavedDetail url=\(request.url?.absoluteString ?? "nil")")
        let (data, response) = try await perform(request)
        guard let http = response as? HTTPURLResponse else {
            throw QuailAPIError.badResponse
        }
        print("[Quail] QuailAPI.fetchExtraSavedDetail status=\(http.statusCode) bytes=\(data.count)")
        guard http.statusCode == 200 else {
            if http.statusCode == 401 || http.statusCode == 403 {
                throw QuailAPIError.unauthorized
            }
        ...
        do {
            ...
            if let text = String(data: data, encoding: .utf8) {
                print("[Quail] QuailAPI.extraSavedDetailDecodeFailure bodyPrefix=\(String(text.prefix(800)))")
            }
            throw QuailAPIError.decodingFailed
        }
    }
    ...
    func fetchBankInfo() async throws -> BankInfoPayload {
        ...
        print("[Quail] QuailAPI.fetchBankInfo url=\(request.url?.absoluteString ?? "nil")")
        let (data, response) = try await perform(request)
        guard let http = response as? HTTPURLResponse else {
            throw QuailAPIError.badResponse
        }
        print("[Quail] QuailAPI.fetchBankInfo status=\(http.statusCode) bytes=\(data.count)")
        guard http.statusCode == 200 else {
            if http.statusCode == 401 || http.statusCode == 403 {
                throw QuailAPIError.unauthorized
            }
        ...
        do {
            ...
            if let text = String(data: data, encoding: .utf8) {
                print("[Quail] QuailAPI.bankInfoDecodeFailure bodyPrefix=\(String(text.prefix(800)))")
            }
            throw QuailAPIError.decodingFailed
        }
    }
    ...
    func fetchAccountInfo(accountID: Int) async throws -> AccountInfoPayload {
        ...
        print("[Quail] QuailAPI.fetchAccountInfo url=\(request.url?.absoluteString ?? "nil")")
        let (data, response) = try await perform(request)
        guard let http = response as? HTTPURLResponse else {
            throw QuailAPIError.badResponse
        }
        print("[Quail] QuailAPI.fetchAccountInfo status=\(http.statusCode) bytes=\(data.count)")
        guard http.statusCode == 200 else {
            if http.statusCode == 401 || http.statusCode == 403 {
                throw QuailAPIError.unauthorized
            }
        ...
        do {
            ...
            if let text = String(data: data, encoding: .utf8) {
                print("[Quail] QuailAPI.accountInfoDecodeFailure bodyPrefix=\(String(text.prefix(800)))")
            }
            throw QuailAPIError.decodingFailed
        }
    }
    ...
    func fetchAccountSeries(accountID: Int, start: String, end: String) async throws -> [ChartSeriesPoint] {
        ...
        print("[Quail] QuailAPI.fetchAccountSeries url=\(request.url?.absoluteString ?? "nil")")
        let (data, response) = try await perform(request)
        guard let http = response as? HTTPURLResponse else {
            throw QuailAPIError.badResponse
        }
        print("[Quail] QuailAPI.fetchAccountSeries status=\(http.statusCode) bytes=\(data.count)")
        guard http.statusCode == 200 else {
            if http.statusCode == 401 || http.statusCode == 403 {
                throw QuailAPIError.unauthorized
            }
        ...
        do {
            ...
            if let text = String(data: data, encoding: .utf8) {
                print("[Quail] QuailAPI.accountSeriesDecodeFailure bodyPrefix=\(String(text.prefix(800)))")
            }
            throw QuailAPIError.decodingFailed
        }
    }
    ...
    func fetchAccountTransactionsRange(accountID: Int, start: String, end: String, limit: Int = 500) async throws -> AccountTransactionsRangePayload {
        ...
        print("[Quail] QuailAPI.fetchAccountTransactionsRange url=\(request.url?.absoluteString ?? "nil")")
        let (data, response) = try await perform(request)
        guard let http = response as? HTTPURLResponse else {
            throw QuailAPIError.badResponse
        }
        print("[Quail] QuailAPI.fetchAccountTransactionsRange status=\(http.statusCode) bytes=\(data.count)")
        guard http.statusCode == 200 else {
            if http.statusCode == 401 || http.statusCode == 403 {
                throw QuailAPIError.unauthorized
            }
        ...
        do {
            ...
            if let text = String(data: data, encoding: .utf8) {
                print("[Quail] QuailAPI.accountTransactionsRangeDecodeFailure bodyPrefix=\(String(text.prefix(800)))")
            }
            throw QuailAPIError.decodingFailed
        }
    }
    ...
    func fetchNotifications(limit: Int = 100) async throws -> [NotificationItemPayload] {
        ...
        print("[Quail] QuailAPI.fetchNotifications url=\(request.url?.absoluteString ?? "nil")")
        let (data, response) = try await perform(request)
        guard let http = response as? HTTPURLResponse else {
            throw QuailAPIError.badResponse
        }
        print("[Quail] QuailAPI.fetchNotifications status=\(http.statusCode) bytes=\(data.count)")
        guard http.statusCode == 200 else {
            if http.statusCode == 401 || http.statusCode == 403 {
                throw QuailAPIError.unauthorized
            }
        ...
        do {
            ...
            if let text = String(data: data, encoding: .utf8) {
                print("[Quail] QuailAPI.notificationsDecodeFailure bodyPrefix=\(String(text.prefix(800)))")
            }
            throw QuailAPIError.decodingFailed
        }
    }
    ...
    func fetchNotificationDetail(id: Int) async throws -> NotificationDetailPayload {
        ...
        let (data, response) = try await perform(request)
        guard let http = response as? HTTPURLResponse else { throw QuailAPIError.badResponse }
        guard http.statusCode == 200 else {
            if http.statusCode == 401 || http.statusCode == 403 { throw QuailAPIError.unauthorized }
            throw QuailAPIError.badResponse
        }
        do {
            ...
            if let text = String(data: data, encoding: .utf8) {
                print("[Quail] QuailAPI.notificationDetailDecodeFailure bodyPrefix=\(String(text.prefix(800)))")
            }
            throw QuailAPIError.decodingFailed
        }
    }
    ...
    func fetchAdminErrorNotifications(limit: Int = 200) async throws -> [AdminErrorNotificationPayload] {
        ...
        do {
            ...
            if let text = String(data: data, encoding: .utf8) {
                print("[Quail] QuailAPI.errorNotificationsDecodeFailure bodyPrefix=\(String(text.prefix(800)))")
            }
            throw QuailAPIError.decodingFailed
        }
    }
    ...
    func fetchPendingUsers() async throws -> [PendingUserPayload] {
        ...
        do {
            ...
            if let text = String(data: data, encoding: .utf8) {
                print("[Quail] QuailAPI.pendingUsersDecodeFailure bodyPrefix=\(String(text.prefix(800)))")
            }
            throw QuailAPIError.decodingFailed
        }
    }
    ...
    func fetchUpcomingWindow(daysAhead: Int = 30, accountID: Int? = nil) async throws -> [UpcomingEventPayload] {
        ...
        print("[Quail] QuailAPI.fetchUpcomingWindow url=\(request.url?.absoluteString ?? "nil")")
        let (data, response) = try await perform(request)
        guard let http = response as? HTTPURLResponse else {
            throw QuailAPIError.badResponse
        }
        print("[Quail] QuailAPI.fetchUpcomingWindow status=\(http.statusCode) bytes=\(data.count)")
        guard http.statusCode == 200 else {
            if http.statusCode == 401 || http.statusCode == 403 {
                throw QuailAPIError.unauthorized
            }
        ...
        do {
            ...
            if let text = String(data: data, encoding: .utf8) {
                print("[Quail] QuailAPI.upcomingDecodeFailure bodyPrefix=\(String(text.prefix(800)))")
            }
            throw QuailAPIError.decodingFailed
        }
    }
    ...
    func createTransaction(accountID: Int, amount: Double, merchant: String, status: String, date: String) async throws -> Bool {
        ...
        print("[Quail] QuailAPI.createTransaction url=\(request.url?.absoluteString ?? "nil")")
        let (data, response) = try await perform(request)
        guard let http = response as? HTTPURLResponse else {
            throw QuailAPIError.badResponse
        }
        print("[Quail] QuailAPI.createTransaction status=\(http.statusCode) bytes=\(data.count)")
        guard http.statusCode == 200 else {
            if http.statusCode == 401 || http.statusCode == 403 {
                throw QuailAPIError.unauthorized
            }
    ...
    func fetchCategoryTransactions(category: String, start: String, end: String, limit: Int = 100) async throws -> [TransactionItem] {
        ...
        print("[Quail] QuailAPI.fetchCategoryTransactions url=\(request.url?.absoluteString ?? "nil")")
        let (data, response) = try await perform(request)
        guard let http = response as? HTTPURLResponse else {
            throw QuailAPIError.badResponse
        }
        print("[Quail] QuailAPI.fetchCategoryTransactions status=\(http.statusCode) bytes=\(data.count)")
        guard http.statusCode == 200 else {
            if http.statusCode == 401 || http.statusCode == 403 {
                throw QuailAPIError.unauthorized
            }
        ...
        do {
            ...
            if let text = String(data: data, encoding: .utf8) {
                print("[Quail] QuailAPI.categoryTransactionsDecodeFailure bodyPrefix=\(String(text.prefix(800)))")
            }
            throw QuailAPIError.decodingFailed
        }
    }
    ...
    func fetchTransactionDetail(txId: String) async throws -> TransactionDetailPayload {
        ...
        print("[Quail] QuailAPI.fetchTransactionDetail url=\(request.url?.absoluteString ?? "nil")")
        let (data, response) = try await perform(request)
        guard let http = response as? HTTPURLResponse else {
            throw QuailAPIError.badResponse
        }
        print("[Quail] QuailAPI.fetchTransactionDetail status=\(http.statusCode) bytes=\(data.count)")
        guard http.statusCode == 200 else {
            if http.statusCode == 401 || http.statusCode == 403 {
                throw QuailAPIError.unauthorized
            }
        ...
        do {
            ...
            if let text = String(data: data, encoding: .utf8) {
                print("[Quail] QuailAPI.transactionDetailDecodeFailure bodyPrefix=\(String(text.prefix(800)))")
            }
            throw QuailAPIError.decodingFailed
        }
    }
    ...
    func setInterestRate(accountID: Int, ratePercent: Double, effectiveDate: String, note: String) async throws -> Bool {
        ...
        print("[Quail] QuailAPI.setInterestRate url=\(request.url?.absoluteString ?? "nil")")
        let (data, response) = try await perform(request)
        guard let http = response as? HTTPURLResponse else {
            throw QuailAPIError.badResponse
        }
        print("[Quail] QuailAPI.setInterestRate status=\(http.statusCode) bytes=\(data.count)")
        guard http.statusCode == 200 else {
            if http.statusCode == 401 || http.statusCode == 403 {
                throw QuailAPIError.unauthorized
            }
    ...
    func verifyAccountBalance(accountID: Int, verifiedDate: String) async throws -> Bool {
        ...
        print("[Quail] QuailAPI.verifyAccountBalance url=\(request.url?.absoluteString ?? "nil")")
        let (data, response) = try await perform(request)
        guard let http = response as? HTTPURLResponse else {
            throw QuailAPIError.badResponse
        }
        print("[Quail] QuailAPI.verifyAccountBalance status=\(http.statusCode) bytes=\(data.count)")
        guard http.statusCode == 200 else {
            if http.statusCode == 401 || http.statusCode == 403 {
                throw QuailAPIError.unauthorized
            }
    ...
    func updateTransactionCategory(txId: String, category: String) async throws -> Bool {
        ...
        print("[Quail] QuailAPI.updateTransactionCategory url=\(request.url?.absoluteString ?? "nil")")
        let (data, response) = try await perform(request)
        guard let http = response as? HTTPURLResponse else {
            throw QuailAPIError.badResponse
        }
        print("[Quail] QuailAPI.updateTransactionCategory status=\(http.statusCode) bytes=\(data.count)")
        guard http.statusCode == 200 else {
            if http.statusCode == 401 || http.statusCode == 403 {
                throw QuailAPIError.unauthorized
            }
    ...
    func updateTransactionMeta(txId: String, status: String, postedDate: String) async throws -> Bool {
        ...
        print("[Quail] QuailAPI.updateTransactionMeta url=\(request.url?.absoluteString ?? "nil")")
        let (data, response) = try await perform(request)
        guard let http = response as? HTTPURLResponse else {
            throw QuailAPIError.badResponse
        }
        print("[Quail] QuailAPI.updateTransactionMeta status=\(http.statusCode) bytes=\(data.count)")
        guard http.statusCode == 200 else {
            if http.statusCode == 401 || http.statusCode == 403 {
                throw QuailAPIError.unauthorized
            }
    ...
    func invertTransactionAmount(txId: String) async throws -> Double {
        ...
        print("[Quail] QuailAPI.invertTransactionAmount url=\(request.url?.absoluteString ?? "nil")")
        let (data, response) = try await perform(request)
        guard let http = response as? HTTPURLResponse else {
            throw QuailAPIError.badResponse
        }
        print("[Quail] QuailAPI.invertTransactionAmount status=\(http.statusCode) bytes=\(data.count)")
        guard http.statusCode == 200 else {
            if http.statusCode == 401 || http.statusCode == 403 {
                throw QuailAPIError.unauthorized
            }
    ...
    func ignoreTransaction(txId: String, ignored: Bool) async throws -> Bool {
        ...
        print("[Quail] QuailAPI.ignoreTransaction url=\(request.url?.absoluteString ?? "nil")")
        let (data, response) = try await perform(request)
        guard let http = response as? HTTPURLResponse else {
            throw QuailAPIError.badResponse
        }
        print("[Quail] QuailAPI.ignoreTransaction status=\(http.statusCode) bytes=\(data.count)")
        guard http.statusCode == 200 else {
            if http.statusCode == 401 || http.statusCode == 403 {
                throw QuailAPIError.unauthorized
            }
    ...
    func deleteTransaction(txId: String) async throws -> Bool {
        ...
        print("[Quail] QuailAPI.deleteTransaction url=\(request.url?.absoluteString ?? "nil")")
        let (data, response) = try await perform(request)
        guard let http = response as? HTTPURLResponse else {
            throw QuailAPIError.badResponse
        }
        print("[Quail] QuailAPI.deleteTransaction status=\(http.statusCode) bytes=\(data.count)")
        guard http.statusCode == 200 else {
            if http.statusCode == 401 || http.statusCode == 403 {
                throw QuailAPIError.unauthorized
            }
    ...
    func fetchMonthBudget(year: Int?, month: Int?) async throws -> MonthBudgetPayload {
        ...
        print("[Quail] QuailAPI.fetchMonthBudget url=\(request.url?.absoluteString ?? "nil")")
        let (data, response) = try await perform(request)
        guard let http = response as? HTTPURLResponse else {
            throw QuailAPIError.badResponse
        }
        print("[Quail] QuailAPI.fetchMonthBudget status=\(http.statusCode) bytes=\(data.count)")
        guard http.statusCode == 200 else {
            if http.statusCode == 401 || http.statusCode == 403 {
                throw QuailAPIError.unauthorized
            }
        ...
        do {
            ...
            if let text = String(data: data, encoding: .utf8) {
                print("[Quail] QuailAPI.monthBudgetDecodeFailure bodyPrefix=\(String(text.prefix(800)))")
            }
            throw QuailAPIError.decodingFailed
        }
    }
    ...
    func fetchLESProfile() async throws -> LESProfilePayload {
        ...
        do {
            ...
            if let text = String(data: data, encoding: .utf8) {
                print("[Quail] QuailAPI.lesProfileDecodeFailure bodyPrefix=\(String(text.prefix(800)))")
            }
            throw QuailAPIError.decodingFailed
        }
    }
    ...
    func saveLESProfile(_ profile: LESProfile) async throws -> LESProfilePayload {
        ...
        do {
            ...
            if let text = String(data: data, encoding: .utf8) {
                print("[Quail] QuailAPI.saveLESProfileDecodeFailure bodyPrefix=\(String(text.prefix(800)))")
            }
            throw QuailAPIError.decodingFailed
        }
    }
    ...
    func fetchLESPaychecks(year: Int, month: Int, profile: LESProfile) async throws -> LESPaychecksPayload {
        ...
        do {
            ...
            if let text = String(data: data, encoding: .utf8) {
                print("[Quail] QuailAPI.lesPaychecksDecodeFailure bodyPrefix=\(String(text.prefix(800)))")
            }
            throw QuailAPIError.decodingFailed
        }
    }
    ...
    func fetchPageBudget(year: Int, month: Int, recalc: Bool = false) async throws -> PageBudgetPayload {
        ...
        print("[Quail] QuailAPI.fetchPageBudget url=\(request.url?.absoluteString ?? "nil")")
        let (data, response) = try await perform(request)
        guard let http = response as? HTTPURLResponse else {
            throw QuailAPIError.badResponse
        }
        print("[Quail] QuailAPI.fetchPageBudget status=\(http.statusCode) bytes=\(data.count)")
        guard http.statusCode == 200 else {
            if http.statusCode == 401 || http.statusCode == 403 { throw QuailAPIError.unauthorized }
            throw QuailAPIError.badResponse
        }
        do {
            ...
            if let text = String(data: data, encoding: .utf8) {
                print("[Quail] QuailAPI.pageBudgetDecodeFailure bodyPrefix=\(String(text.prefix(800)))")
            }
            throw QuailAPIError.decodingFailed
        }
    }
    ...
    func fetchRoundUpSettings() async throws -> RoundUpSettingsPayload {
        ...
        print("[Quail] QuailAPI.fetchRoundUpSettings url=\(request.url?.absoluteString ?? "nil")")
        let (data, response) = try await perform(request)
        guard let http = response as? HTTPURLResponse else { throw QuailAPIError.badResponse }
        print("[Quail] QuailAPI.fetchRoundUpSettings status=\(http.statusCode) bytes=\(data.count)")
        guard http.statusCode == 200 else {
            if http.statusCode == 401 || http.statusCode == 403 { throw QuailAPIError.unauthorized }
            throw QuailAPIError.badResponse
        }
        do {
            ...
            if let text = String(data: data, encoding: .utf8) {
                print("[Quail] QuailAPI.roundupsDecodeFailure bodyPrefix=\(String(text.prefix(800)))")
            }
            throw QuailAPIError.decodingFailed
        }
    }
    ...
    func saveRoundUpSettings(enabled: Bool) async throws -> Bool {
        ...
        print("[Quail] QuailAPI.saveRoundUpSettings url=\(request.url?.absoluteString ?? "nil")")
        let (data, response) = try await perform(request)
        guard let http = response as? HTTPURLResponse else { throw QuailAPIError.badResponse }
        print("[Quail] QuailAPI.saveRoundUpSettings status=\(http.statusCode) bytes=\(data.count)")
        guard http.statusCode == 200 else {
            if http.statusCode == 401 || http.statusCode == 403 { throw QuailAPIError.unauthorized }
            throw QuailAPIError.badResponse
        }
    ...
    func saveSavingsGoal(mode: String, value: Double) async throws -> Bool {
        ...
        print("[Quail] QuailAPI.saveSavingsGoal url=\(request.url?.absoluteString ?? "nil")")
        let (data, response) = try await perform(request)
        guard let http = response as? HTTPURLResponse else { throw QuailAPIError.badResponse }
        print("[Quail] QuailAPI.saveSavingsGoal status=\(http.statusCode) bytes=\(data.count)")
        guard http.statusCode == 200 else {
            if http.statusCode == 401 || http.statusCode == 403 { throw QuailAPIError.unauthorized }
            throw QuailAPIError.badResponse
        }
    ...
    func upsertBudgetGroup(year: Int, month: Int, name: String, allocated: Double, cap: Double?, categories: [String]) async throws -> Bool {
        ...
        print("[Quail] QuailAPI.upsertBudgetGroup url=\(request.url?.absoluteString ?? "nil")")
        let (data, response) = try await perform(request)
        guard let http = response as? HTTPURLResponse else { throw QuailAPIError.badResponse }
        print("[Quail] QuailAPI.upsertBudgetGroup status=\(http.statusCode) bytes=\(data.count)")
        guard http.statusCode == 200 else {
            if http.statusCode == 401 || http.statusCode == 403 { throw QuailAPIError.unauthorized }
            throw QuailAPIError.badResponse
        }
    ...
    func deleteBudgetGroup(year: Int, month: Int, name: String) async throws -> Bool {
        ...
        print("[Quail] QuailAPI.deleteBudgetGroup url=\(request.url?.absoluteString ?? "nil")")
        let (data, response) = try await perform(request)
        guard let http = response as? HTTPURLResponse else { throw QuailAPIError.badResponse }
        print("[Quail] QuailAPI.deleteBudgetGroup status=\(http.statusCode) bytes=\(data.count)")
        guard http.statusCode == 200 else {
            if http.statusCode == 401 || http.statusCode == 403 { throw QuailAPIError.unauthorized }
            throw QuailAPIError.badResponse
        }
    ...
    func createFund(name: String, targetAmount: Double, targetDate: String?, cadence: String, contribAmount: Double) async throws -> Bool {
        ...
        print("[Quail] QuailAPI.createFund url=\(request.url?.absoluteString ?? "nil")")
        let (data, response) = try await perform(request)
        guard let http = response as? HTTPURLResponse else { throw QuailAPIError.badResponse }
        print("[Quail] QuailAPI.createFund status=\(http.statusCode) bytes=\(data.count)")
        guard http.statusCode == 200 else {
            if http.statusCode == 401 || http.statusCode == 403 { throw QuailAPIError.unauthorized }
            throw QuailAPIError.badResponse
        }
    ...
    func updateFund(id: Int, name: String, targetAmount: Double, targetDate: String?, cadence: String, contribAmount: Double, isActive: Bool = true) async throws -> Bool {
        ...
        print("[Quail] QuailAPI.updateFund url=\(request.url?.absoluteString ?? "nil")")
        let (data, response) = try await perform(request)
        guard let http = response as? HTTPURLResponse else { throw QuailAPIError.badResponse }
        print("[Quail] QuailAPI.updateFund status=\(http.statusCode) bytes=\(data.count)")
        guard http.statusCode == 200 else {
            if http.statusCode == 401 || http.statusCode == 403 { throw QuailAPIError.unauthorized }
            throw QuailAPIError.badResponse
        }
    ...
    func adjustFund(id: Int, amount: Double, note: String) async throws -> Bool {
        ...
        print("[Quail] QuailAPI.adjustFund url=\(request.url?.absoluteString ?? "nil")")
        let (data, response) = try await perform(request)
        guard let http = response as? HTTPURLResponse else { throw QuailAPIError.badResponse }
        print("[Quail] QuailAPI.adjustFund status=\(http.statusCode) bytes=\(data.count)")
        guard http.statusCode == 200 else {
            if http.statusCode == 401 || http.statusCode == 403 { throw QuailAPIError.unauthorized }
            throw QuailAPIError.badResponse
        }
    ...
    func deleteFund(id: Int) async throws -> Bool {
        ...
        print("[Quail] QuailAPI.deleteFund url=\(request.url?.absoluteString ?? "nil")")
        let (data, response) = try await perform(request)
        guard let http = response as? HTTPURLResponse else { throw QuailAPIError.badResponse }
        print("[Quail] QuailAPI.deleteFund status=\(http.statusCode) bytes=\(data.count)")
        guard http.statusCode == 200 else {
            if http.statusCode == 401 || http.statusCode == 403 { throw QuailAPIError.unauthorized }
            throw QuailAPIError.badResponse
        }
    ...
    func fetchDayLimit(recalc: Bool = false) async throws -> DayLimitPayload {
        ...
        print("[Quail] QuailAPI.fetchDayLimit url=\(request.url?.absoluteString ?? "nil")")
        let (data, response) = try await perform(request)
        guard let http = response as? HTTPURLResponse else {
            throw QuailAPIError.badResponse
        }
        print("[Quail] QuailAPI.fetchDayLimit status=\(http.statusCode) bytes=\(data.count)")
        guard http.statusCode == 200 else {
            if http.statusCode == 401 || http.statusCode == 403 {
                throw QuailAPIError.unauthorized
            }
        ...
        do {
            ...
            if let text = String(data: data, encoding: .utf8) {
                print("[Quail] QuailAPI.dayLimitDecodeFailure bodyPrefix=\(String(text.prefix(800)))")
            }
            throw QuailAPIError.decodingFailed
        }
    }
    ...
    func fetchSpentSoFarBreakdown(start: String, end: String) async throws -> SpentSoFarBreakdownPayload {
        ...
        print("[Quail] QuailAPI.fetchSpentSoFarBreakdown url=\(request.url?.absoluteString ?? "nil")")
        let (data, response) = try await perform(request)
        guard let http = response as? HTTPURLResponse else {
            throw QuailAPIError.badResponse
        }
        print("[Quail] QuailAPI.fetchSpentSoFarBreakdown status=\(http.statusCode) bytes=\(data.count)")
        guard http.statusCode == 200 else {
            if http.statusCode == 401 || http.statusCode == 403 {
                throw QuailAPIError.unauthorized
            }
        ...
        do {
            ...
            if let text = String(data: data, encoding: .utf8) {
                print("[Quail] QuailAPI.spentBreakdownDecodeFailure bodyPrefix=\(String(text.prefix(800)))")
            }
            throw QuailAPIError.decodingFailed
        }
    }
    ...
    func fetchSpentSoFarTransactions(category: String, start: String, end: String, limit: Int = 500) async throws -> [TransactionItem] {
        ...
        print("[Quail] QuailAPI.fetchSpentSoFarTransactions url=\(request.url?.absoluteString ?? "nil")")
        let (data, response) = try await perform(request)
        guard let http = response as? HTTPURLResponse else {
            throw QuailAPIError.badResponse
        }
        print("[Quail] QuailAPI.fetchSpentSoFarTransactions status=\(http.statusCode) bytes=\(data.count)")
        guard http.statusCode == 200 else {
            if http.statusCode == 401 || http.statusCode == 403 {
                throw QuailAPIError.unauthorized
            }
        ...
        do {
            ...
            if let text = String(data: data, encoding: .utf8) {
                print("[Quail] QuailAPI.spentTxDecodeFailure bodyPrefix=\(String(text.prefix(800)))")
            }
            throw QuailAPIError.decodingFailed
        }
    }
    ...
    func fetchChartSeries(mode: ChartMode, start: String, end: String) async throws -> [ChartSeriesPoint] {
        ...
        print("[Quail] QuailAPI.fetchChartSeries url=\(request.url?.absoluteString ?? "nil")")
        let (data, response) = try await perform(request)
        guard let http = response as? HTTPURLResponse else {
            throw QuailAPIError.badResponse
        }
        print("[Quail] QuailAPI.fetchChartSeries status=\(http.statusCode) bytes=\(data.count)")
        guard http.statusCode == 200 else {
            if http.statusCode == 401 || http.statusCode == 403 {
                throw QuailAPIError.unauthorized
            }
        ...
        do {
            ...
            if let text = String(data: data, encoding: .utf8) {
                print("[Quail] QuailAPI.chartDecodeFailure bodyPrefix=\(String(text.prefix(800)))")
            }
            if let decodingError = error as? DecodingError {
                print("[Quail] QuailAPI.chartDecodingError=\(Self.describe(decodingError))")
            } else {
                print("[Quail] QuailAPI.chartDecodeFailure error=\(error.localizedDescription)")
            }
            throw QuailAPIError.decodingFailed
        }
    }
    ...
    func fetchSpendingSeries(start: String, end: String) async throws -> [ChartSeriesPoint] {
        ...
        print("[Quail] QuailAPI.fetchSpendingSeries url=\(request.url?.absoluteString ?? "nil")")
        let (data, response) = try await perform(request)
        guard let http = response as? HTTPURLResponse else { throw QuailAPIError.badResponse }
        print("[Quail] QuailAPI.fetchSpendingSeries status=\(http.statusCode) bytes=\(data.count)")
        guard http.statusCode == 200 else {
            if http.statusCode == 401 || http.statusCode == 403 { throw QuailAPIError.unauthorized }
            throw QuailAPIError.badResponse
        }
        do {
            ...
            if let text = String(data: data, encoding: .utf8) {
                print("[Quail] QuailAPI.spendingSeriesDecodeFailure bodyPrefix=\(String(text.prefix(800)))")
            }
            throw QuailAPIError.decodingFailed
        }
    }
    ...
    func fetchSpendingCategoryTotals(start: String, end: String) async throws -> [SpendingCategoryTotalPayload] {
        ...
        print("[Quail] QuailAPI.fetchSpendingCategoryTotals url=\(request.url?.absoluteString ?? "nil")")
        let (data, response) = try await perform(request)
        guard let http = response as? HTTPURLResponse else { throw QuailAPIError.badResponse }
        print("[Quail] QuailAPI.fetchSpendingCategoryTotals status=\(http.statusCode) bytes=\(data.count)")
        guard http.statusCode == 200 else {
            if http.statusCode == 401 || http.statusCode == 403 { throw QuailAPIError.unauthorized }
            throw QuailAPIError.badResponse
        }
        do {
            ...
            if let text = String(data: data, encoding: .utf8) {
                print("[Quail] QuailAPI.spendingCategoryTotalsDecodeFailure bodyPrefix=\(String(text.prefix(800)))")
            }
            throw QuailAPIError.decodingFailed
        }
    }
    ...
    func fetchUnknownMerchantRange(start: String, end: String) async throws -> UnknownMerchantRangePayload {
        ...
        print("[Quail] QuailAPI.fetchUnknownMerchantRange url=\(request.url?.absoluteString ?? "nil")")
        let (data, response) = try await perform(request)
        guard let http = response as? HTTPURLResponse else { throw QuailAPIError.badResponse }
        print("[Quail] QuailAPI.fetchUnknownMerchantRange status=\(http.statusCode) bytes=\(data.count)")
        guard http.statusCode == 200 else {
            if http.statusCode == 401 || http.statusCode == 403 { throw QuailAPIError.unauthorized }
            throw QuailAPIError.badResponse
        }
        do {
            ...
            if let text = String(data: data, encoding: .utf8) {
                print("[Quail] QuailAPI.unknownMerchantRangeDecodeFailure bodyPrefix=\(String(text.prefix(800)))")
            }
            throw QuailAPIError.decodingFailed
        }
    }
    ...
    func fetchSpendingUnbudgetedSafeRange(start: String, end: String) async throws -> SpendingUnbudgetedSafeRangePayload {
        ...
        print("[Quail] QuailAPI.fetchSpendingUnbudgetedSafeRange url=\(request.url?.absoluteString ?? "nil")")
        let (data, response) = try await perform(request)
        guard let http = response as? HTTPURLResponse else { throw QuailAPIError.badResponse }
        print("[Quail] QuailAPI.fetchSpendingUnbudgetedSafeRange status=\(http.statusCode) bytes=\(data.count)")
        guard http.statusCode == 200 else {
            if http.statusCode == 401 || http.statusCode == 403 { throw QuailAPIError.unauthorized }
            throw QuailAPIError.badResponse
        }
        do {
            ...
            if let text = String(data: data, encoding: .utf8) {
                print("[Quail] QuailAPI.spendingUnbudgetedSafeRangeDecodeFailure bodyPrefix=\(String(text.prefix(800)))")
            }
            throw QuailAPIError.decodingFailed
        }
    }
    ...
    func fetchSpendingUnbudgetedDay(day: String) async throws -> SpendingUnbudgetedDayPayload {
        ...
        print("[Quail] QuailAPI.fetchSpendingUnbudgetedDay url=\(request.url?.absoluteString ?? "nil")")
        let (data, response) = try await perform(request)
        guard let http = response as? HTTPURLResponse else { throw QuailAPIError.badResponse }
        print("[Quail] QuailAPI.fetchSpendingUnbudgetedDay status=\(http.statusCode) bytes=\(data.count)")
        guard http.statusCode == 200 else {
            if http.statusCode == 401 || http.statusCode == 403 { throw QuailAPIError.unauthorized }
            throw QuailAPIError.badResponse
        }
        do {
            ...
            if let text = String(data: data, encoding: .utf8) {
                print("[Quail] QuailAPI.spendingUnbudgetedDayDecodeFailure bodyPrefix=\(String(text.prefix(800)))")
            }
            throw QuailAPIError.decodingFailed
        }
    }
    ...
    func fetchData(path: String, queryItems: [URLQueryItem] = [], method: String = "GET", jsonBody: [String: Any]? = nil) async throws -> Data {
        ...
        print("[Quail] QuailAPI.fetchData url=\(request.url?.absoluteString ?? "nil")")
        let (data, response) = try await perform(request)
        guard let http = response as? HTTPURLResponse else {
            throw QuailAPIError.badResponse
        }
        print("[Quail] QuailAPI.fetchData status=\(http.statusCode) bytes=\(data.count)")
        guard http.statusCode == 200 else {
            if http.statusCode == 401 || http.statusCode == 403 {
                throw QuailAPIError.unauthorized
            }
    ...
    func fetchVehicleTxCandidates(kind: String, recordId: Int) async throws -> [VehicleTxCandidate] {
        ...
        do {
            ...
            print("[Quail] fetchVehicleTxCandidates decode error: \(error)\nraw: \(raw)")
            throw error
        }
    }
    ...
    private func makeRequest(url: URL, method: String = "GET", jsonBody: [String: Any]? = nil) -> URLRequest {
        ...
        request.setValue("Quail/1.0", forHTTPHeaderField: "User-Agent")
    ...
    private func perform(_ request: URLRequest) async throws -> (Data, URLResponse) {
        ...
        do {
            ...
            do {
                ...
                print("[Quail] QuailAPI.transportError=\(error.localizedDescription)")
                throw QuailAPIError.transport(error)
            }
        } catch {
            NetworkLogger.shared.log(method: method, path: path, status: -1, duration: Date().timeIntervalSince(started), error: error.localizedDescription)
            print("[Quail] QuailAPI.transportError=\(error.localizedDescription)")
    ...
    private func checkedData(for request: URLRequest) async throws -> Data {
        ...
        guard (200...299).contains(http.statusCode) else {
            ...
            throw QuailAPIError.transport(NSError(domain: "QuailAPI", code: http.statusCode, userInfo: [
