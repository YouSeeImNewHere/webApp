...
final class SavedPlacesStore: ObservableObject {
    ...
    func refresh() async {
        ...
        guard let raw = try? await QuailAPI.shared.fetchSavedPlaceLists() else { return }
        lists = raw.compactMap(SavedPlaceList.from)
    }

    func loadPlaces(for listId: Int) async {
        guard let raw = try? await QuailAPI.shared.fetchSavedPlaces(listId: listId) else { return }
        placesByList[listId] = raw.compactMap(SavedPlace.from)
    }

    func createList(name: String, emoji: String, color: String) async {
        guard let raw = try? await QuailAPI.shared.createSavedPlaceList(name: name, emoji: emoji, color: color),
              let list = SavedPlaceList.from(raw) else { return }
        lists.append(list)
    }

    func updateList(_ list: SavedPlaceList) async {
        try? await QuailAPI.shared.updateSavedPlaceList(list.id, name: list.name, emoji: list.emoji, color: list.color)
    ...
    func deleteList(_ list: SavedPlaceList) async {
        try? await QuailAPI.shared.deleteSavedPlaceList(list.id)
    ...
    func savePlace(listId: Int, name: String, address: String, latitude: Double?, longitude: Double?, notes: String = "") async -> SavedPlace? {
        guard let raw = try? await QuailAPI.shared.savePlace(listId: listId, name: name, address: address, latitude: latitude, longitude: longitude, notes: notes),
    ...
    func deletePlace(_ place: SavedPlace) async {
        try? await QuailAPI.shared.deleteSavedPlace(place.id)
