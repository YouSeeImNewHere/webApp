...
enum ShortcutImportProcessor {
    ...
    static func process(
        ...
        do {
            let preview = try await QuailAPI.shared.fetchCsvPreview(fileURL: stagedURL)
            ...
            let jobID = try await QuailAPI.shared.importCsvMappedAsync(fileURL: stagedURL, fields: mappingFields(from: preset, accountID: accountID))
            var jobStatus: CsvJobStatusPayload
            while true {
                try await Task.sleep(nanoseconds: 800_000_000)
                jobStatus = try await QuailAPI.shared.pollCsvJob(jobID: jobID)
                progress?(ShortcutImportProgressSnapshot(
...
struct QuailAppShortcuts: AppShortcutsProvider {
