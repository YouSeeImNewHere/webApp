...
GMAIL_PUSH_REQUIRE_OIDC = (os.getenv("GMAIL_PUSH_REQUIRE_OIDC", "false") or "").lower() in ("1", "true", "yes")
GMAIL_PUSH_OIDC_AUDIENCE = (os.getenv("GMAIL_PUSH_OIDC_AUDIENCE", "") or "").strip()
GMAIL_PUSH_OIDC_EMAIL = (os.getenv("GMAIL_PUSH_OIDC_EMAIL", "") or "").strip().lower()
OAUTH_TOKEN_ENCRYPTION_KEY = (os.getenv("OAUTH_TOKEN_ENCRYPTION_KEY", "") or "").strip()
OAUTH_TOKEN_ENCRYPTION_KEYS = (os.getenv("OAUTH_TOKEN_ENCRYPTION_KEYS", "") or "").strip()
