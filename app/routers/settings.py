...
def set_notification_settings(body: NotificationPrefsIn, request: Request):
    _ensure_app_settings_pg()
    rows = query_db(
        "SELECT value_json FROM app_settings WHERE key = %s LIMIT 1",
        (scoped_key("notification_prefs"),),
...
@router.get("/settings/cache-versions")
def get_cache_versions(request: Request):
    tid = current_tenant_id()
    if MULTI_TENANT_ENABLED:
        tenant_id, _ = _require_approved_session_user(request)
        tid = int(tenant_id)
    ...
    home_version = int(home_snapshot_version_for_tenant(tid) or 0)
    widget_version = 0
    try:
        from app.routers.page_payloads import _widget_version_for_tenant_cached
        widget_version = int(_widget_version_for_tenant_cached(tid) or 0)
    except Exception:
        widget_version = int(_refresh_widget_cache_for_tenant_best_effort(int(tid)) or 0)
    ...
    return {
        "ok": True,
        "tenant_id": int(tid),
        "home_snapshot_version": int(home_version),
        "widget_version": int(widget_version),
    }
