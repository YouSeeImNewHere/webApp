...
const WIDGET_TOKEN = "wgt_REPLACED_BY_SERVER";
...
const CACHE_PATH = fmLocal.joinPath(
  fmLocal.documentsDirectory(),
  `finance_widget_cache_${cacheKeyForToken()}.json`
);
...
function saveCache(obj) {
  const now = Date.now();
  try { fmLocal.writeString(CACHE_PATH, JSON.stringify({ ts: now, data: obj })); } catch (_) {}
}
function loadCache() {
  if (!fmLocal.fileExists(CACHE_PATH)) return null;
  try { return JSON.parse(fmLocal.readString(CACHE_PATH)); } catch (_) { return null; }
}
...
if (!fam) {
  try {
    ...
    if (!isValidPayload(resp)) throw new Error("invalid_widget_payload");
    saveCache(resp);
...
const cache = loadCache();
if (cache && isValidPayload(cache.data) && isExpectedTenantPayload(cache.data)) {
  payload = cache.data;
  cacheAgeMin = cacheAgeMinutes(cache);
}

const cachedVersion =
  (payload && Number.isFinite(Number(payload.widget_version)))
    ? Number(payload.widget_version)
    : null;

try {
  let resp = await fetchSummary(cachedVersion);
  if (resp && !isExpectedTenantPayload(resp)) throw new Error("tenant_mismatch");
  if (resp && resp.ok === true && resp.update_required === true) {
  ...
  if (resp && resp.ok === true && resp.changed === true) {
    if (!isValidPayload(resp)) return finish(updateRequiredWidget(), "Update required");
    if (!isExpectedTenantPayload(resp)) return finish(updateRequiredWidget(), "Tenant mismatch");
    payload = resp;
    usedCache = false;
    cacheAgeMin = 0;
    saveCache(resp);
